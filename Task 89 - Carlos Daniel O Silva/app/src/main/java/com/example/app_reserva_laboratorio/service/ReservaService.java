...
public class ReservaService {
    ...
    private boolean existeConflito(Reserva reservaParaVerificar) {
        ...
        // 1. Pega o "Lab Pai" da reserva que queremos fazer (ex: "LAB - H403")
        ...
        for (Reserva existente : reservasExistentes) {
            ...
            // Pula a verificação se for a MESMA reserva (importante para o "Editar")
            ...
            // 2. Pega o "Lab Pai" da reserva existente
            ...
            // 3. Checa se são do mesmo "Lab Pai" (ex: ambos são "LAB - H403")
            if (!labPaiParaVerificar.equals(labPaiExistente)) {
                continue; // Laboratórios principais diferentes, sem conflito.
            }

            // Se chegou aqui, é no mesmo DIA e no mesmo LAB PAI (ex: LAB - H403).

            // 4. Checa a sobreposição de horário
            ...
            // (Ex: Nova reserva é "lab_h403" e existente é "est_12" -> CONFLITO)
            // (Ex: Nova reserva é "est_12" e existente é "lab_h403" -> CONFLITO)
            if (ehReservaDeLabInteiro || existenteEhLabInteiro) {
                return true; // CONFLITO!
            }

            // Se NENHUMA é do lab inteiro, são duas estações. (Ex: "est_12" vs "est_10")
            // Só há conflito se forem a MESMA estação.
            if (reservaParaVerificar.getLaboratorioId().equals(existente.getLaboratorioId())) {
                return true; // CONFLITO! (Ex: "est_12" vs "est_12")
            }
    ...
    private String getLabPai(String nomeCompleto) {
        if (nomeCompleto.contains(" - ")) {
            ...
            if (partes.length > 1) {
                return partes[1]; // Retorna "LAB - H403"
            }
        }
        return nomeCompleto; // Retorna "LAB - H401"
    }
}